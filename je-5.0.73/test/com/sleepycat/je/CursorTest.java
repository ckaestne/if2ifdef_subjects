...
public class CursorTest extends DualTestCase {
    ...
    @Test
    public void testGetNextAfterPrefix() {
        ...
        for (final byte[] d : data) {
            final DatabaseEntry e = new DatabaseEntry(d);
            final OperationStatus status = db.put(null, e, e);
            assertSame(OperationStatus.SUCCESS, status);
        }

        final Cursor cursor = db.openCursor(null, null);

        checkNextAfterPrefix(cursor, data, 0, 7);
        checkNextAfterPrefix(cursor, data, 1, 4);
        checkNextAfterPrefix(cursor, data, 2, 3);
        checkNextAfterPrefix(cursor, data, 3, 4);
        checkNextAfterPrefix(cursor, data, 4, 7);
        checkNextAfterPrefix(cursor, data, 5, 6);
        checkNextAfterPrefix(cursor, data, 6, 7);
        checkNextAfterPrefix(cursor, data, 7, -1);
    ...
    private void checkNextAfterPrefix(Cursor cursor,
                                      byte[][] data,
                                      int inputIndex,
                                      int outputIndex) {
        final DatabaseEntry key = new DatabaseEntry(data[inputIndex]);
        final DatabaseEntry dataEntry = new DatabaseEntry();
        final OperationStatus status =
            cursor.getNextAfterPrefix(key, dataEntry, null);
        if (outputIndex < 0) {
            assertSame(OperationStatus.NOTFOUND, status);
            return;
        }
        assertSame(OperationStatus.SUCCESS, status);
        if (!Arrays.equals(data[outputIndex], key.getData())) {
            fail
                ("inputIndex=" + inputIndex + " outputIndex=" + outputIndex +
                 " expectData=" + Arrays.toString(data[outputIndex]) +
                 " gotData=" + Arrays.toString(key.getData()));
        }
    }
}

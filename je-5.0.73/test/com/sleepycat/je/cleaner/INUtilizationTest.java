...
public class INUtilizationTest extends CleanerTestBase {
    ...
    public INUtilizationTest(boolean multiSubDir) {
    ...
    public static List<Object[]> genParams() {
        ...
        return getEnv(new boolean[] {false});
    }
    ...
    private void closeEnv(boolean doCheckpoint,
         ...
         *
         * Do not expect accurate LN size counting if dups is true, because we
         * do not fetch the previous LN during update/delete and we estimate
         * the size.
         */
        VerifyUtils.verifyUtilization
            (envImpl, expectAccurateObsoleteLNCount,
             expectAccurateObsoleteLNSize && !dups,
    ...
    /**
     * Tests that BINDelta utilization counting works.
     */
    @Test
    ...
    /**
     * Similar to testRecovery, but tests BINDeltas.
     */
    @Test
    public void testBINDeltaRecovery()
        ...
        /*
         * Log explicitly since we have no way to do a partial checkpoint.
         * The BINDelta is logged provisionally and the IN non-provisionally.
         */
        ...
        /* Save current BINDelta and IN files. */
        ...
        /* Expect to obsolete the BINDelta and IN, but not the full BIN. */
        expectObsolete(fullBinFile, false);
        expectObsolete(deltaBinFile, true);
        expectObsolete(inFile, true);
        assertEquals(fullBinFile, getFullBINFile(cursor));
        ...
        /* Expect that recovery counts only BINDelta and IN as obsolete. */
        expectObsolete(fullBinFile, false);
        expectObsolete(deltaBinFile, true);
        expectObsolete(inFile, true);
        expectObsolete(deltaBinFile2, false);
        ...
        /* Expect to obsolete the full BIN, the BINDelta and the IN. */
        expectObsolete(fullBinFile, true);
        expectObsolete(deltaBinFile, true);
        expectObsolete(inFile, true);

        /* Save current BIN, BINDelta and IN files. */
        ...
        /* Expect that recovery counts BIN, BINDelta and IN as obsolete. */
        expectObsolete(fullBinFile, true);
        expectObsolete(deltaBinFile, true);
        expectObsolete(inFile, true);
        expectObsolete(fullBinFile2, false);
    ...
    private long getDeltaBINFile(Cursor cursor) {
        long lsn = TestUtils.getBIN(cursor).getLastDeltaVersion();

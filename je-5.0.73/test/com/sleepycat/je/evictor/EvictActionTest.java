...
public class EvictActionTest extends TestBase {
    ...
    public void testSmallCacheSettings()
        ...
        /*
         * RequiredEvictBytes must be read from the stats after
         * the operation has run, before the act of evicting.
         */
        EnvironmentStats stats = env.getStats(null);
        assertTrue("requiredEvict=" + stats.getRequiredEvictBytes(),
                   stats.getRequiredEvictBytes() > 400 * 1024);
    ...
    public void testCleaningAfterMappingTreeEviction()
        ...
        envConfig.setConfigParam(EnvironmentConfig.CLEANER_MIN_UTILIZATION,
                                 "75");
        /* The bug is provoked only with lazy migration disabled. */
        envConfig.setConfigParam(EnvironmentConfig.CLEANER_LAZY_MIGRATION,
                                                 "false");
        openEnv(envConfig);
        EnvironmentImpl envImpl = DbInternal.getEnvironmentImpl(env);

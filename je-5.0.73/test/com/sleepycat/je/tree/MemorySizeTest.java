...
public class MemorySizeTest extends DualTestCase {
        ...
        for (IN in : inList) {
            if (in instanceof BIN &&
                ...
                try {
                    ...
                    fail("No LNs evicted and no DINs present.");
                } finally {
                    bin.releaseLatch();
                }
            }
    ...
    private void checkCount(int expectedCount)
        ...
        while (cursor.getNext(new DatabaseEntry(), new DatabaseEntry(),
            ...
            /*
             * To make memory size counting easily testable for a duplicates
             * DB, we force the LN to be fetched when it is counted.
             */
            CursorImpl cursorImpl = DbInternal.getCursorImpl(cursor);
            cursorImpl.latchBIN();
            try {
                cursorImpl.getBIN().fetchTarget(cursorImpl.getIndex());
            } finally {

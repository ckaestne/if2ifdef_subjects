...
public class EnvMultiSubDirTest extends TestBase {
    ...
    private void doTestWork(boolean deferredWrite, boolean transactional)
        ...
        EnvironmentConfig envConfig = 
            createEnvConfig(deferredWrite, transactional);
    ...
    private EnvironmentConfig createEnvConfig(boolean deferredWrite, 
    ...
    public void testLogFilesDirCheck()
        ...
        try {
            new Environment(envHome, createEnvConfig(false, false));
            fail("Expected to see exceptions.");
        } catch (EnvironmentFailureException e) {
            /* Expectged exceptions. */
        } catch (Exception e) {
    ...
    private void checkDbContents(Database db, int start, int end)
        ...
        for (int i = start; i <= end; i++) {
            ...
            assertEquals
                (StringBinding.entryToString(data), dataValue + dataValue);
        }
    }
}

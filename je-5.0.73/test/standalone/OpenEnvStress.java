 ...
 * will check that all the environments are using the same SharedEvictor.
 */
    ...
    private void doSharedCacheCheck(OpenEnvAndWorkThread[] threads) {
        ...
        for (OpenEnvAndWorkThread thread : threads) {
            if (evictor == null) {
                ...
                if (!(evictor instanceof SharedEvictor)) {
                    throw new IllegalStateException
                        ("Shared cache environments must use SharedEvictor.");
                }
            } else {
